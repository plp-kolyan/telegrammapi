...
избегайте True - payload_internal в этом случае не будет добавленн при первоначальном рендеренге кнопок 

Полезную нагрузку которая попала в контекст сообщений можно посмотреть 
...
Для того чтобы добавить полезную нагрузку нужно прописать в шаблон ответа вот так    
